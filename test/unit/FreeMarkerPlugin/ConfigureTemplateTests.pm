...
<#if firstTime?? && firstTime == 1>(we will solve this in a minute)</#if>';
    ...
    $result   = $parsed;
    $expected = 'Status:<span class=\'configureStatusErrors\'>1</span><span class=\'configureStatusWarnings\'>1</span>';
...
sub test_pagebegin {
    ...
    $template  = '<form method="post" action="${formAction}" enctype="multipart/form-data" name="update">
<input type="hidden" name="time" value="${time}"  />';
    ...
    $result   = $parsed;
    $expected = '<form method="post" action="configure" enctype="multipart/form-data" name="update">
...
sub test_feedback {
    ...
    $template  = '<#if messageType == 4><div class="foswikiNotification"> Password changed </div></#if>
...
';
    ...
    $parsed = $parser->parse($template, {
    	messageType => 4,
    	changesList => [{ key => 'A', value => 'a' }, { key => 'B', value => 'b'} ],
    	frontpageUrl => 'http://a/bin/c',
    	configureUrl => 'http://x/bin/z'
    });
    $result   = $parsed;
...
sub test_section {
    ...
    $template  = '<div id=\'${id}Body\' class=\'${bodyClass}\'><a name=\'${id}\'><!--//--></a>
...
<p class="foswikiClear"></p>',
    ...
    $result   = $parsed;
    $expected = '<div id=\'securityBody\' class=\'main\'><a name=\'security\'><!--//--></a>
...
';
    ...
    $result   = $parsed;
    $expected = '<div class=\'foswikiNotification enableWhenSomethingChanged foswikiHidden\'>
<span class="foswikiAlert"><strong>You made some changes! Consider saving them first.</strong></span>
