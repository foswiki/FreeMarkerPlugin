...
sub new {
    ...
    my ($self) = $class->SUPER::new(
        ...
        yystates  => [
            ...
            {    #State 315
                ACTIONS => { "</#"             => 334 },
                GOTOS   => { 'tag_close_start' => 335 }
            },
            {    #State 316
                ACTIONS => { "whitespace" => 19 },
                DEFAULT => -19,
                GOTOS   => { 'whitespace' => 336 }
            },
            {    #State 317
                ACTIONS => { "</#"             => 334 },
                GOTOS   => { 'tag_close_start' => 337 }
            },
            {    #State 318
                ACTIONS => { "whitespace" => 19 },
                DEFAULT => -19,
                GOTOS   => { 'whitespace' => 338 }
            },
            {    #State 319
                ACTIONS => { ">"            => 173 },
                GOTOS   => { 'tag_open_end' => 339 }
            },
            {    #State 320
                ACTIONS => {
                    'variable_verbatim' => 15,
            ...
            {    #State 337
                ACTIONS => { "assign"               => 353 },
                GOTOS   => { 'directive_assign_end' => 352 }
            },
            {    #State 338
                DEFAULT => -100
            },
            ...
            {    #State 351
                ACTIONS => { ">"             => 283 },
                GOTOS   => { 'tag_close_end' => 359 }
            },
            {    #State 352
                ACTIONS => { ">"             => 283 },
                GOTOS   => { 'tag_close_end' => 360 }
            },
            {    #State 353
                DEFAULT => -110
            },
            ...
            {    #State 361
                ACTIONS => { "</#"             => 334 },
                GOTOS   => { 'tag_close_start' => 367 }
            },
            {    #State 362
                ACTIONS => { ">"             => 283 },
                GOTOS   => { 'tag_close_end' => 368 }
            },
            {    #State 363
                DEFAULT => -115
            },
...
#line 331 "FreeMarkerGrammar.yp"
                {
                    my $content = $_[8] || '';
                    $_[0]->{_data}->{_fmMacros}->{ $_[3] }->{contents} =
                      $content;

                    my @params;
                    @params = split( / /, $_[4] ) if $_[4];
                    my %paramsHash = map { $_ => 1 } @params;
                    $_[0]->{_data}->{_fmMacros}->{ $_[3] }->{params} =
                      \%paramsHash;
...
#line 369 "FreeMarkerGrammar.yp"
                {
                    print "Invalid macro parameter:$_[1]\n"
                      if (
                        !$_[0]->{_workingData}->{validMacroParams}->{ $_[1] } );
                    if ( _isString( $_[3] ) ) {
...
#line 384 "FreeMarkerGrammar.yp"
                {
                    $_[0]->{_workingData}->{validMacroParams} =
                      $_[0]->{_data}->{_fmMacros}->{ $_[4] }->{params};
                  }
            ],
            [    #Rule 126
                '@11-6', 0,
...
#line 391 "FreeMarkerGrammar.yp"
                {
                    my $parsed =
                      $_[0]->_parse(
                        $_[0]->{_data}->{_fmMacros}->{ $_[4] }->{contents} );
...
#line 414 "FreeMarkerGrammar.yp"
                {
                    ...
                    if ($format) {
                        $result = $_[0]->_renderList( $key, $listData, $format )
                          if $listData;
                    }
                    return $result;
                  }
...
# to prevent parsing of nested if blocks first, first parse level 0, and after that nested if blocks
                        ...
                        push( @{ $_[0]->{_workingData}->{ifBlocks} },
                            $ifBlock );
                        my $ifBlockId =
                          scalar @{ $_[0]->{_workingData}->{ifBlocks} } - 1;
                        return '___ifblock' . $ifBlockId . '___';
                    }
                    $_[0]->{_workingData}->{ifBlocks} = ()
                      if $_[0]->{_workingData}->{ifLevel} == 0;
                  }
            ],
            [    #Rule 135
...
#line 654 "FreeMarkerGrammar.yp"
                {
                    ...
                    if ($isStringSort) {
                        @sorted =
                          sort { lc $$a{$key} cmp lc $$b{$key} } @{ $_[1] };
                    }
                    else {
...
#line 673 "FreeMarkerGrammar.yp"
                {
                    ...
                    $_[0]->{_workingData}->{ $_[1] }->{'seqData'} ||=
                      _arrayAsHash( $_[1], 1 );
                    my $index =
                      $_[0]->{_workingData}->{ $_[1] }->{'seqData'}->{ $_[5] };
...
#line 684 "FreeMarkerGrammar.yp"
                {
                    ...
                    $_[0]->{_workingData}->{ $_[1] }->{'seqData'} ||=
                      _arrayAsHash( $_[1], 1 );
                    return 1
                      if defined $_[0]->{_workingData}->{ $_[1] }->{'seqData'}
                          ->{ $_[5] };
                    return 0;
                  }
            ],
